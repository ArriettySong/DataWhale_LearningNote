...
> IForest也有一些缺点，比如不适用于特别高维的数据。由于每次切数据空间都是随机选取一个维度和该维度的随机一个特征，建完树后仍然有大量的维度没有被使用，导致算法可靠性降低。此时推荐降维后使用，或者考虑使用One Class SVM。
>
> iForest仅对即全局稀疏点敏感，不擅长处理局部的相对稀疏点 。
>
> 而One Class SVM对于中小型的数据分析，尤其是训练样本不是特别海量的时候用起来经常会比iForest顺手，因此比较适合做原型分析。

HBOS：可扩展性强

AutoEncoder：适合数据量大、特征多的场景
...
1. 异常账号/行为的实时监控，为快速捕捉，可以采用基于统计学类的耗时较低的方法，对疑似异常的账号/简历/职位进行弱管控。同时，将该部分数据**异步**推送至耗时较高的复杂模型进行二次识别和强管控。
2. 随着时间推移，业务数据在持续变化，异常检测的模型要时常更新。
3. 看了两天，只看了比较基础的，偏向传统机器学习的异常检测算法，发现有二三十种，且不说，时序异常检测和图异常检测的算法还没看。。差点快被吓倒了。。 工业环境下，要求的是简单、快速、有效、可解释性强！所以，没有必要贪多啃下所有算法，按需选择进行学习即可。并且学术上有对很多普通的异常检测算法进行优化，要学的是思路，不是死记硬背。带着新思路回到工业环境下尝试吧！
...
> 有的聚类算法并不是把所有的点都归到簇里去，对于远离任何一个簇的点，算法本身会认为是噪音点，相当于是异常。但聚类算法倾向于发现簇而不是异常点，当一小团点彼此相似且都是异常时，那么算法就会把它识别成簇。识别异常的准确率取决于聚类是否能更好的捕捉到真实的簇分布。
>
> 
>
> 作者：尼古丁布

> - **基于模型的方法（Model-based Approaches）【非常认可】**：
>
> - - 理论：对正常数据建造一个模型，那么异常值就是不能fit这个模型的。
>   - 举例：基于统计的方法，基于深度的方法，基于偏差的方法，基于子空间划分的方法。
>
> 
>
> 作者：尼古丁布

> 现在正在看“Outlier Analysis”，发现**ensemble的方式更好用一些**，对feature和data进行bagging之后再利用多种模型输出异常分值，或加权或平均，或者干脆加入到原始特征里，都是比较有效的方法，在实际应用中，模型的精度和召回都提高了五个百分点以上。对于异常检测这个大坑，还是要继续填。
>
> 
>
> 作者：尼古丁布
> 链接：https://www.zhihu.com/question/280696035/answer/416508926
> 来源：知乎
> 著作权归作者所有。商业转载请联系作者获得授权，非商业转载请注明出处。



本文重要参考原文见：https://github.com/datawhalechina/team-learning-data-mining/blob/master/AnomalyDetection/%E4%B8%80%E3%80%81%E6%A6%82%E8%BF%B0.md
...
[COPOD：用「统计」+「机器学习」检测异常](https://zhuanlan.zhihu.com/p/338189299)

[异常检测（anomaly detection）](https://blog.csdn.net/u012328159/article/details/51462942)
